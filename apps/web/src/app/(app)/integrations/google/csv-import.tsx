    ...
    return (
      <div className="animate-rise-in rounded-xl border border-accent/30 bg-accent/[0.06] p-4">
          ...
          <details className="mt-3">
            ...
            <ul className="mt-2 space-y-0.5">
              {resultado.ignoradas.map((item) => (
                <li key={item.linha} className="text-rotulo text-ink-mute">
        ...
        <button
          ...
          className="focus-ring mt-3 rounded text-apoio text-ink-mute underline decoration-line underline-offset-4 hover:text-ink"
        >
    ...
    <div>
      {!previa ? (
        <label className="focus-within:ring-accent/15 flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-line bg-panel-soft/40 px-4 py-6 text-center transition-colors hover:border-accent/40 focus-within:ring-4">
          ...
          <span className="text-corpo font-medium text-ink">Escolher relatório</span>
          <span className="mt-0.5 text-rotulo text-ink-mute">
        ...
        <div className="animate-rise-in rounded-xl border border-line bg-panel-soft/40 p-4">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className="mb-1 block text-rotulo text-ink-mute">Coluna da data</span>
              <select
                value={colunaData}
                onChange={(e) => setColunaData(Number(e.target.value))}
                className="h-9 w-full rounded-lg border border-line bg-panel px-2 text-apoio text-ink focus:border-accent focus:outline-none"
              >
            ...
            <label className="block">
              <span className="mb-1 block text-rotulo text-ink-mute">Coluna do gasto</span>
              <select
                value={colunaValor}
            ...
            <table className="w-full text-left text-apoio">
              <thead>
                <tr className="border-b border-line/60">
                  <th className="px-2.5 py-1.5 text-rotulo uppercase tracking-wide text-ink-mute">Data lida</th>
                  <th className="px-2.5 py-1.5 text-rotulo uppercase tracking-wide text-ink-mute">Gasto lido</th>
                </tr>
              </thead>
              <tbody>
