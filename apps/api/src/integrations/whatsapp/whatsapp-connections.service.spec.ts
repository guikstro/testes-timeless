...
describe("WhatsAppConnectionsService", () => {
  function buildService() {
    ...
    const motor = {
      conecta: jest.fn(),
      estado: jest.fn().mockReturnValue("connecting"),
      qrCode: jest.fn().mockResolvedValue({ base64: "data:image/png;base64,QR", code: "code" }),
      numeroConectado: jest.fn().mockReturnValue(null),
      desconecta: jest.fn(),
    };
    ...
    return { service, prisma, encryption, motor };
  }
  ...
  it("desconectar por QR Code desliga o aparelho no motor", async () => {
    const { service, prisma, motor } = buildService();
    prisma.whatsAppConnection.findUnique.mockResolvedValue({ organizationId: "org-1", provider: "EVOLUTION", instanceName: "org-org-1" });

    await service.disconnect("org-1");

    expect(motor.desconecta).toHaveBeenCalledWith("org-org-1");
  });
});
